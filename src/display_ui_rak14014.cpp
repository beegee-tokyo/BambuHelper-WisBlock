...
static void smoothLerp(float &cur, float target)
{
  ...
  if (fabsf(diff) < SNAP_THRESH)
    cur = target;
  else
    cur += diff * SMOOTH_ALPHA;
}
...
static void drawPrinting()
{
  ...
  if (forceRedraw || stateChanged)
  {
    ...
    if (strcmp(s.gcodeState, "RUNNING") == 0)
      badgeColor = CLR_GREEN;
    else if (strcmp(s.gcodeState, "PAUSE") == 0)
      badgeColor = CLR_YELLOW;
    else if (strcmp(s.gcodeState, "FAILED") == 0)
      badgeColor = CLR_RED;
    else if (strcmp(s.gcodeState, "PREPARE") == 0)
      badgeColor = CLR_BLUE;
